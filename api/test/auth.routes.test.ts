...
function createTestApp(controllerOverrides = {}) {
  const controller = {
    ...
    verifyEmail: async (_req, res) => res.status(200).json({ success: true }),
    resendVerification: async (_req, res) => res.status(200).json({ success: true }),
    ...controllerOverrides,
  };

  const app = express();
  app.use(express.json());
  app.use('/api/auth', createAuthRouter(controller as any));
  app.use(notFound);
