...
function difficultyToRatingRange(difficulty: string | null | undefined) {
...
function inRatingRange(question: any, range: { min: number; max: number } | null) {
...
function pickShuffled(ids: string[], count: number): string[] {
...
export class SessionStartService {
  ...
  private questionOptionRepository: any;
  private quizRepository: any;
  private quizAccessRepository: any;
  private friendRepository: any;
  private storySessionRepository: any;
  private storyLevelRepository: any;
  private storyLevelPoolRepository: any;
  private storyService: any;
  private millionaireLadderRepository: any;
  private modeQuestionPoolRepository: any;
  private classicCategoryPoolRepository: any;
  private classicCategoryLevelRepository: any;
  private classicCategoryLevelPoolRepository: any;
    ...
    if (this.modeQuestionPoolRepository) {
      try {
        ...
        for (let off = 0; off < shuffled.length && picked.length < count; off += batchSize) {
          ...
          const rows = (await this.quizQuestionRepository.listByIds(batchIds)) as any[];
          const map = new Map<string, any>(rows.map((q) => [q.id, q]));
  ...
  async snapshotSessionQuestions(sessionId: string, sourceQuestions: any[]) {
    ...
    const bySourceId = new Map<string, any>(
      (createdSessionQuestions as any[]).map((sq) => [sq.source_question_id, sq])
    );
  ...
  async appendSessionQuestions(
    sessionId: string,
    sourceQuestions: any[],
    { orderIndexStart = 0 }: { orderIndexStart?: number } = {}
  ) {
  ...
  async primeGuestSessionCache(
    ...
    mode: string,
    sourceQuestions: any[],
    { started_at = null }: { started_at?: string | null } = {}
  ) {
