...
import CreateQuizHero from '@/features/CreateQuiz/components/CreateQuizHero';
import CreateQuizLockedCard from '@/features/CreateQuiz/components/CreateQuizLockedCard';
import { useCreateQuizPublishValidation } from '@/features/CreateQuiz/hooks/useCreateQuizPublishValidation';

export type CreateQuizUser = {
  id?: string;
} | null;

export type CreateQuizPageProps = {
  user?: CreateQuizUser;
  onRequireAuth?: () => void;
  onNavigateHome?: () => void;
};
...
/**
 * Quiz builder page for creating, editing, and publishing user quizzes.
 * @param user Current user snapshot (builder is auth-gated).
 * @param onRequireAuth Callback to request authentication.
...
export default function CreateQuizPage({
  ...
  const { publishIssues, canPublish } = useCreateQuizPublishValidation(quiz, questions);
          ...
          <button
            ...
            onClick={async () => {
              if (publishIssues.length > 0) {
                setError(STRINGS.CREATE_QUIZ.publish.fixBefore(publishIssues));
                return;
              }
              setError('');
              setBusy(true);
  ...
  return (
    <div style={CreateQuizPageStyle.page}>
      <div style={CreateQuizPageStyle.container}>
        ...
        {!canUseBuilder ? (
          <CreateQuizLockedCard onRequireAuth={onRequireAuth} />
        ) : (
          <>
            {!!error && (
