...
import PlaySessionTopBar from '@/features/PlaySession/components/PlaySessionTopBar';
import PlaySessionResults from '@/features/PlaySession/components/PlaySessionResults';
import PlaySessionMillionaireView from '@/features/PlaySession/components/PlaySessionMillionaireView';
import PlaySessionDefaultQuestionView from '@/features/PlaySession/components/PlaySessionDefaultQuestionView';
import PlaySessionStoryView from '@/features/PlaySession/components/PlaySessionStoryView';
import {
  clampPct,
  clampStars,
  computeStoryOutcomeFromCounts,
  formatMoney,
} from '@/features/PlaySession/lib/playSessionHelpers';
import PlaySessionErrorCard from '@/features/PlaySession/components/PlaySessionErrorCard';
import { usePlaySessionShare } from '@/features/PlaySession/hooks/usePlaySessionShare';

type SessionUser = {
  id?: string;
} | null;

type SessionQuestion = any;
type SessionReview = any;
type SessionResult = any;

export type PlaySessionPageProps = {
  sessionId?: string | null;
  user?: SessionUser;
  // eslint-disable-next-line no-unused-vars
  onRequireAuth?: (...args: [mode?: string]) => void;
  onBack?: () => void;
  onNavigateHome?: () => void;
  backLabel?: string;
  variant?: 'default' | 'story';
  storyLevelNumber?: number | null;
  classicCategoryId?: string | null;
...
export default function PlaySessionPage({
  ...
  const didPrewarmRefreshRef = useRef(false);
  ...
  const getNextTransitionMs = (q: SessionQuestion) => {
  ...
  const [audiencePoll, setAudiencePoll] = useState<any>(null);
  const [phoneSuggestionOptionId, setPhoneSuggestionOptionId] = useState<string | null>(null);
  ...
  const [answerResult, setAnswerResult] = useState<SessionResult | null>(null);
  ...
  const [storyOutcome, setStoryOutcome] = useState<any>(null);
  const [classicOutcome, setClassicOutcome] = useState<any>(null);
  const [classicLevelsMax, setClassicLevelsMax] = useState<number | null>(null);
  ...
  useEffect(() => {
    // Signed-in users can experience an extra 401->refresh->retry round-trip
    // on the first protected request when the access token is stale.
    // Prewarming refresh in the background reduces that first-question latency
    // without blocking guest flow or changing gameplay behavior.
    if (!user?.id) return;
    if (didPrewarmRefreshRef.current) return;
    didPrewarmRefreshRef.current = true;
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((res) => {
        if (!alive) return;
        const typed = res as { levels?: any[] };
        const list = Array.isArray(typed?.levels) ? typed.levels : [];
        const max = list
  ...
  const timeInfo = useMemo(() => {
    ...
    if (question.mode === 'blitz' && Number.isFinite(blitzRemaining)) {
      ...
      if (question.mode === 'blitz' && Number.isFinite(Number(question?.strikes_remaining))) {
        return `${base} â€¢ ${Number(question.strikes_remaining)} strikes left`;
      }
      return base;
    }
  ...
  const { shareMessage, setShareMessage, shareText, doShare } = usePlaySessionShare({
    scoreDisplay,
    modeLabel,
    isStory,
    storyLevelNumber,
    classicLevelNum: classicResult?.levelNum ?? null,
  });
  ...
  const submit = async (chosenOptionId: string) => {
  ...
  const triggerLifeline = async (lifeline_type: string) => {
  ...
  return (
    <div style={PlaySessionStyle.page}>
      ...
      >
        ...
        <PlaySessionErrorCard error={error} />
