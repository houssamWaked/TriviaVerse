...
import DuelQuestionOptions from '@/features/DuelPlay/components/DuelQuestionOptions';
import { useDuelStartCountdown } from '@/features/DuelPlay/hooks/useDuelStartCountdown';
import type { DuelPlayProps, DuelState } from '@/features/DuelPlay/types';
...
export default function DuelPlayPage({
  ...
  const { stateReceivedAtMs, msUntilStart, markStateReceived } = useDuelStartCountdown({
    serverMsUntilStart,
    isCompleted,
    questionStartedAt: question?.started_at,
  });
  ...
  useEffect(() => {
    // Signed-in duel users can hit an initial 401->refresh->retry delay.
    // Prewarm refresh once in background to reduce first state-load latency.
    if (!user?.id) return;
